...
uint256 hashGenesisBlock("0x784b69c9944a75bc15132fe488cc919d0618334ad36a6a0de06266969910c654");
...
const string strMessageMagic = "Jaycoin Signed Message:\n";
...
{
    // Jaycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Jaycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jaycoin genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1441715711;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4066614;
	    ...
	    assert(block.hashMerkleRoot == uint256("0xf147ec6025907234135bc7f9c5f59050227fe7b3c17afe3c5013fbb0d6890865"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x24, 0xcd, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JaycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JaycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JaycoinMiner(CWallet *pwallet)
{
    printf("JaycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Jaycoin-miner");
    ...
    try { loop {
        ...
        printf("Running JaycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JaycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JaycoinMiner, pwallet));
}
