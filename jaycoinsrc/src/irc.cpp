...
// Copyright (c) 2011-2012 Jaycoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #JaycoinTEST0\r");
            Send(hSocket, "WHO #JaycoinTEST0\r");
        } else {
            // randomly join #Jaycoin00-#Jaycoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Jaycoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Jaycoin%02d\r", channel_number).c_str());
        }
